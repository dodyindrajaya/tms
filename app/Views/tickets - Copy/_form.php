...
<div class="form-group"><label>Passenger *</label><select name="passenger_id" required><option value="">-- Select Passenger --</option><?php foreach($passengers as $p): ?><option value="<?=$p['id']?>" <?=$v('passenger_id')==$p['id']?'selected':''?>><?=esc($p['full_name'] ?? ($p['name'] ?? ''))?></option><?php endforeach;?></select></div>
