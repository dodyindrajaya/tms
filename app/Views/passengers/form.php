<?php $p=$passenger??[];$v=fn($f,$d='')=>old($f,$p[$f]??$d);?><?php
$flashSuccess = session()->getFlashdata('success');
$flashError = session()->getFlashdata('error');
?>
<div class="tms-page">

<div class="tms-page-head"><div><div class="eyebrow">MASTER DATA / PASSENGERS</div><h1><?=esc($pageTitle)?></h1><p>Passenger profile for booking and ticketing.</p></div><a class="tms-btn" href="<?=site_url('passengers')?>">Back</a></div>
<?php if($msg=session()->getFlashdata('error')):?><div class="tms-alert tms-alert-danger"><?=esc($msg)?></div><?php endif;?>
<form method="post" action="<?=site_url($isEdit?'passengers/update/'.$p['id']:'passengers/create')?>"><?=csrf_field()?>
<div class="tms-card"><div class="tms-card-head"><div><h2>Passenger Information</h2><p>Reusable passenger master across bookings.</p></div></div><div class="tms-form-grid">
<div class="field"><label>Full Name *</label><input name="full_name" value="<?=esc($v('full_name'))?>" required maxlength="190"></div>
<div class="field"><label>Customer</label><select name="customer_id"><option value="">-- No Customer Link --</option><?php foreach($customers as $c):?><option value="<?=$c['id']?>" <?=((string)$v('customer_id')===(string)$c['id'])?'selected':''?>><?=esc($c['name'])?> (<?=esc($c['customer_code'])?>)</option><?php endforeach;?></select></div>
<div class="field"><label>Gender</label><select name="gender"><option value="">-- Select --</option><?php foreach($genders as $k=>$label):?><option value="<?=$k?>" <?=$v('gender')===$k?'selected':''?>><?=esc($label)?></option><?php endforeach;?></select></div>
<div class="field"><label>Birth Date</label><input type="date" name="birth_date" value="<?=esc($v('birth_date'))?>"></div>
<div class="field"><label>Nationality</label><input name="nationality_code" value="<?=esc($v('nationality_code'))?>" maxlength="2" placeholder="ID"></div>
<div class="field"><label>ID Number</label><input name="id_number" value="<?=esc($v('id_number'))?>" maxlength="50"></div>
<div class="field"><label>Passport No.</label><input name="passport_no" value="<?=esc($v('passport_no'))?>" maxlength="50"></div>
<div class="field"><label>Passport Expiry</label><input type="date" name="passport_expiry" value="<?=esc($v('passport_expiry'))?>"></div>
<div class="field"><label>Phone / WhatsApp</label><input name="phone" value="<?=esc($v('phone'))?>" maxlength="30"></div>
<div class="field"><label>Email</label><input type="email" name="email" value="<?=esc($v('email'))?>" maxlength="190"></div>
<div class="field field-full"><label>Notes</label><textarea name="notes" rows="4"><?=esc($v('notes'))?></textarea></div>
</div></div><div class="tms-form-actions"><a class="tms-btn" href="<?=site_url('passengers')?>">Cancel</a><button class="tms-btn tms-btn-primary"><?=$isEdit?'Save Changes':'Create Passenger'?></button></div>
</form>
