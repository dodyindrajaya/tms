...
<div class="tms-page-head"><div><div class="eyebrow">TRANSACTIONS / TICKETING</div><h1>Ticketing</h1><p>Manage flight, train, bus and ferry tickets linked to bookings.</p></div><a class="tms-btn tms-btn-primary" href="<?=site_url('ticketing/new')?>">+ New Ticket</a></div>
<?php if($flashSuccess):?><div class="tms-alert tms-alert-success"><?=esc($flashSuccess)?></div><?php endif;?>
<?php if($flashError):?><div class="tms-alert tms-alert-danger"><?=esc($flashError)?></div><?php endif;?>
<div class="tms-stats">
<div class="tms-stat"><span>Total Tickets</span><strong><?=number_format($stats['total'])?></strong><small>All ticket records</small></div>
<div class="tms-stat"><span>Active</span><strong><?=number_format($stats['booked'])?></strong><small>Booked / paid / issued</small></div>
<div class="tms-stat"><span>Issued</span><strong><?=number_format($stats['issued'])?></strong><small>Ready for travel</small></div>
<div class="tms-stat"><span>Selling Value</span><strong>Rp <?=number_format($stats['revenue'],0,',','.')?></strong><small>Ticket selling price</small></div>
</div>
<form class="tms-filter" method="get">
<input name="q" value="<?=esc($q)?>" placeholder="Search booking, passenger, PNR/ticket, route or carrier...">
<select name="ticket_type"><option value="">All Types</option><?php foreach($ticketTypes as $k=>$label):?><option value="<?=esc($k)?>" <?=$type===$k?'selected':''?>><?=esc($label)?></option><?php endforeach;?></select>
<select name="status"><option value="">All Status</option><?php foreach($statuses as $k=>$label):?><option value="<?=esc($k)?>" <?=$status===$k?'selected':''?>><?=esc($label)?></option><?php endforeach;?></select>
<button class="tms-btn tms-btn-secondary">Search</button>
</form>
<div class="tms-card"><div class="tms-card-head"><div><h2>Ticket Records</h2><p><?=count($tickets)?> record(s) shown</p></div></div>
<div class="tms-table-wrap"><table class="tms-table"><thead><tr><th>BOOKING</th><th>PASSENGER</th><th>TYPE</th><th>ROUTE</th><th>DEPARTURE</th><th>PNR / TICKET</th><th>STATUS</th><th>SELLING</th><th>ACTION</th></tr></thead><tbody>
<?php if(!$tickets):?><tr><td colspan="9" class="empty">No ticket records found.</td></tr><?php endif;?>
<?php foreach($tickets as $t):?><tr>
<td><strong><?=esc($t['booking_no'])?></strong><small><?=esc($t['booking_code']??'-')?></small></td>
<td><strong><?=esc($t['passenger_name'])?></strong><small><?=esc($t['passenger_code'])?></small></td>
<td><span class="type-pill"><?=esc($ticketTypes[$t['ticket_type']]??ucfirst($t['ticket_type']))?></span></td>
<td><strong><?=esc($t['origin']?:'-')?> → <?=esc($t['destination']?:'-')?></strong></td>
<td><?=esc($t['departure_date']?:'-')?><small><?=esc(substr((string)($t['departure_time']??''),0,5))?></small></td>
<td><?=esc($t['booking_code']?:'-')?><small><?=esc($t['ticket_number']?:'-')?></small></td>
<td><span class="status-pill status-<?=esc($t['status'])?>"><?=esc($statuses[$t['status']]??$t['status'])?></span></td>
<td><strong>Rp <?=number_format((float)$t['selling_price'],0,',','.')?></strong></td>
<td class="actions"><a class="tms-btn tms-btn-sm" href="<?=site_url('ticketing/show/'.$t['id'])?>">View</a><a class="tms-btn tms-btn-sm" href="<?=site_url('ticketing/edit/'.$t['id'])?>">Edit</a></td>
</tr><?php endforeach;?>
</tbody></table></div></div>
